...
class BackendSmokeTest:
    ...
    async def run_health_checks(self):
        ...
        # Test dual health endpoint consistency
        await self._test_dual_health_consistency()

    async def _test_dual_health_consistency(self):
        ...
        try:
            ...
            api_health_result = await self.test_endpoint("/api/health")

            if not (health_result.success and api_health_result.success):
                print("  ❌ One or both health endpoints failed")
                return

            # Parse JSON responses
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.base_url}/health") as resp1:
                    health_json = await resp1.json()
                async with session.get(f"{self.base_url}/api/health") as resp2:
