...
import { createClient, isConfigured as isSupabaseConfigured } from '@/lib/supabase/client'
...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    if (!isSupabaseConfigured) {
      formStatus.setError('OAuth not available', 'Supabase is not configured. Please use email/password login.')
      return
    }
    formStatus.setSubmitting('Redirecting to Google...')
  ...
  const handleGithubLogin = async () => {
    ...
    formStatus.setSubmitting('Redirecting to GitHub...')
    const supabase = createClient()
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'github',
      options: {
          ...
          <div className="mt-6">
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              ...
              <Button
                ...
                disabled={formStatus.status === 'submitting' || !isSupabaseConfigured}
                title={!isSupabaseConfigured ? 'OAuth requires Supabase configuration' : undefined}
              >
