...
class TestDQNNetworkImplementation:
    ...
    def test_numpy_network_prediction(self):
        ...
        network = NumpyDQNModel(state_size=5, action_size=3)
    ...
    def test_numpy_network_training(self):
        ...
        network = NumpyDQNModel(state_size=3, action_size=2, learning_rate=0.1)
...
class TestEnhancedDQNAgent:
    ...
    def test_agent_initialization(self):
        ...
        assert self.agent.action_size == 5
...
class TestQuantumRoutingManager:
    ...
    def test_manager_initialization(self):
        ...
        assert self.manager.quantum_manager is not None
        assert self.manager.advanced_router is not None
        assert self.manager.dqn_agent is not None
    ...
    async def test_routing_strategy_execution(self):
        ...
        decision = await self.manager._execute_routing_strategy(
            request=request,
            strategy=RoutingStrategy.TASK_OPTIMIZED
        )
        
        assert decision.provider is not None
        assert decision.model is not None
        assert "task-optimized" in decision.reasoning
    ...
    async def test_quantum_routing_execution(self):
        ...
        assert result.strategy_used in [RoutingStrategy.TASK_OPTIMIZED, RoutingStrategy.BALANCED]
    ...
    def test_routing_state_conversion(self):
        ...
        assert routing_state.context_type in [
            "code_generation", "analysis", "debugging", "documentation",
            "testing", "planning", "research", "creative", "reasoning", "general"
        ]
    
    def test_routing_decision_scoring(self):
        """Test scoring of routing decisions."""
        from monkey_coder.core.routing import RoutingDecision
        ...
        score = self.manager._score_routing_decision(decision)
...
class TestRouterIntegration:
    ...
    def test_routing_analytics(self):
        ...
        analytics = self.integration.get_routing_analytics()
        ...
        assert stats["quantum_enabled"] == True
    ...
    def test_health_status(self):
        ...
        assert status["quantum_enabled"] == True
        assert status["quantum_manager_healthy"] == True
        assert status["classic_router_healthy"] == True
...
class TestEndToEndIntegration:
    ...
    async def test_complete_routing_workflow(self):
        ...
        analytics = integration.get_routing_analytics()
    ...
    def test_performance_metrics_integration(self):
        ...
        # Get initial metrics
        initial_analytics = integration.get_routing_analytics()
        initial_requests = initial_analytics["routing_statistics"]["total_requests"]
