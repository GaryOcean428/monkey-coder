...
class TestExperienceBufferPerformance:
    ...
    def test_single_operation_performance(self):
        ...
        assert add_time < 0.001, f"add took {add_time:.6f}s, should be < 1ms"
        assert sample_time < 0.010, f"sample took {sample_time:.6f}s, should be < 10ms"
        assert len(batch) == 32
    ...
    def test_concurrent_load_1000_operations(self):
        ...
        def worker_sample_operations():
            ...
            try:
                ...
                for _ in range(operations_per_thread):
                    if len(buffer) >= 10:  # Only sample if enough data
                        start_time = time.perf_counter()
        ...
        assert len(buffer) <= buffer.capacity  # Should not exceed capacity
    ...
    def test_memory_usage_under_load(self):
        ...
        # Buffer should be near capacity (cleanup happens at 90% threshold)
        assert len(buffer) >= buffer.capacity * 0.85, f"Buffer should be near capacity, got {len(buffer)}/{buffer.capacity}"
    ...
    def test_scalability_with_large_buffers(self):
        ...
        for i in range(10000):
            ...
            large_buffer.add(experience)

        # Test sampling performance
        start_time = time.perf_counter()
        small_batch = small_buffer.sample(100)
        small_time = time.perf_counter() - start_time

        start_time = time.perf_counter()
        large_batch = large_buffer.sample(100)
