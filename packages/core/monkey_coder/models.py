...
class SuperClaudeConfig(BaseModel):
    ...
    slash_commands: List[str] = Field(default_factory=list, description="Enabled slash commands")
    context_window: int = Field(default=32768, description="Context window size")
    use_markdown_spec: bool = Field(default=True, description="Use markdown specification")
    custom_instructions: Optional[str] = Field(None, description="Custom persona instructions")
...
class Monkey1Config(BaseModel):
    ...
    coordination_strategy: str = Field(default="collaborative", description="Agent coordination strategy")
    consensus_threshold: float = Field(default=0.7, description="Consensus threshold for decisions")
    enable_reflection: bool = Field(default=True, description="Enable agent reflection")
    max_iterations: int = Field(default=5, description="Maximum orchestration iterations")
...
class Gary8DConfig(BaseModel):
    ...
    parallel_futures: bool = Field(default=True, description="Enable parallel execution")
    collapse_strategy: str = Field(default="weighted_average", description="Quantum collapse strategy")
    quantum_coherence: float = Field(default=0.8, description="Quantum coherence level")
    execution_branches: int = Field(default=3, description="Number of execution branches")
    uncertainty_threshold: float = Field(default=0.1, description="Uncertainty threshold")
...
class ExecuteRequest(BaseModel):
    ...
    superclause_config: SuperClaudeConfig = Field(..., description="SuperClaude configuration")
    monkey1_config: Monkey1Config = Field(default_factory=Monkey1Config, description="Monkey1 configuration")
    gary8d_config: Gary8DConfig = Field(default_factory=Gary8DConfig, description="Gary8D configuration")
    
    # Provider preferences
    preferred_providers: List[ProviderType] = Field(default_factory=list, description="Preferred AI providers")
    model_preferences: Dict[ProviderType, str] = Field(default_factory=dict, description="Model preferences by provider")
...
class UsageMetrics(BaseModel):
    ...
    provider_breakdown: Dict[str, int] = Field(..., description="Token usage by provider")
...
class ExecutionResult(BaseModel):
    ...
    quantum_collapse_info: Dict[str, Any] = Field(default_factory=dict, description="Quantum collapse information")
...
class ExecuteResponse(BaseModel):
    ...
    # Integration info
    superclause_routing: Dict[str, Any] = Field(default_factory=dict, description="SuperClaude routing info")
    monkey1_orchestration: Dict[str, Any] = Field(default_factory=dict, description="Monkey1 orchestration info")
    gary8d_execution: Dict[str, Any] = Field(default_factory=dict, description="Gary8D execution info")
...
class UsageResponse(BaseModel):
    ...
    # Detailed breakdowns
    provider_breakdown: List[ProviderUsage] = Field(..., description="Usage by provider")
    execution_stats: Dict[str, Any] = Field(..., description="Execution statistics")
    rate_limit_status: List[RateLimitStatus] = Field(..., description="Rate limit status")
    
    # Trends and insights
    daily_usage: List[Dict[str, Any]] = Field(default_factory=list, description="Daily usage breakdown")
    cost_trends: Dict[str, Any] = Field(default_factory=dict, description="Cost trend analysis")
...
class ExecutionError(Exception):
    ...
    def __init__(self, message: str, error_code: str = "EXECUTION_ERROR", details: Optional[Dict[str, Any]] = None):
...
class ProviderError(Exception):
    ...
    def __init__(self, message: str, provider: str, error_code: str = "PROVIDER_ERROR", details: Optional[Dict[str, Any]] = None):
        self.message = message
...
def get_available_models(provider: Optional[ProviderType] = None) -> Dict[str, List[str]]:
    """
